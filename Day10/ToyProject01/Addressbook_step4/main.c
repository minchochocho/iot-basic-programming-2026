...
static void update_contact() {
	...
	if (buf[0]!='\0') {	// 이 조건을 걸어 값이 입력되었을때만 메모리를 덮어쓰게함
		...
		contacts[index - 1].name[NAME_LEN - 1] = '\0';	// 안정빵용 추가. 글자를 MAX 적으면 \0이 들어가야 문쟈열의 끝을 알려줌
	...
	if (buf[0] != '\0') {	// 엔터를 친게 아니면, 내용을 입력했으면
		...
		contacts[index - 1].name[PHONE_LEN - 1] = '\0';
	}
	...
	if (buf[0] != '\0') {	// 엔터를 친게 아니면, 내용을 입력했으면
		...
		contacts[index - 1].name[ADDR_LEN - 1] = '\0';	
	}
	...
	if (buf[0] != '\0') {	// 엔터를 친게 아니면, 내용을 입력했으면
		...
		contacts[index - 1].name[EMAIL_LEN - 1] = '\0';
	}
	...
	if (buf[0] != '\0') {	// 엔터를 친게 아니면, 내용을 입력했으면
		...
		contacts[index - 1].name[MEMO_LEN - 1] = '\0';
	}
