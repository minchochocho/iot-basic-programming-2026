...
// step 1
static void print_menu();	// static - 이 소스 내에서만 쓰겠다
static int read_menu();
static void read_line(char* buf, int size);

// step 2
static void add_contact();
static void list_contact();
...
static void sort_by_name();		// 이름순 정렬  qsort() 함수 사용, stdlib.h에 포함
static int cmp_contact_name(const void* a, const char* b);
...
static int contains_pipe(const char* s) {
	...
	return(strchr(s, '|') != NULL);	// 발견되 |이후의 문자열이 리턴
}
...
static int read_menu() {
	...
	printf("선택 > ");
	if (scanf("%d", &choice) != 1) {	// 숫자면 초이스에 저장하고 숫자가 아니면 -1을 반환

		// 버퍼 비우기 : scanf는 숫자만 가져가고 버퍼에 엔터를 남기기에
		// 찌꺼기를 치우지 않으면 다음에 오는 입력함수에서 엔터를 입력으로 착각해 빈문자열을 읽을 수 잇음
		while ((ch = getchar()) != '\n' && ch != EOF) { }	
		// 엔터를 만날때까지 모든 문자를 읽어서 버림
		return -1;
	}
...
static void read_line(char* buf, int size) {
	if ((fgets(buf, size, stdin) == NULL)) {	// size만큼 stdin에서 문자열을 읽어 buf에 저장 => 이게 NULL일때
		buf[0] = '\n';
		return;
	}
	// strcspn(buf,"\n") = buf 문자열 내에서 \n을 찾아서 그 위치값을 리턴
...
// 메뉴 1번 기능. 연락처 추가
static void add_contact() {
	//if (count >= MAX_CONTACTS) {	// 지정된 크기만큼만
	//	puts("주소록 최대인원인 100명에 도달했습니다.");
	//	return;
	//}
	...
	printf("이름[최대10자] : ");
	read_line(contacts[count].name, NAME_LEN);	// read_line(시작주소, 최대크기)
...
static void list_contact() {
	...
	// contacts 배열이 들어있는 주소록 출력
	for (i = 0; i < count; i++) {	// 저장된 인원수 만큼 루프 돌리기
		printf("%4d | %-11s | %-15s | %-30s | %-25s\n",
...
static void search_contact() {
	...
	printf("검색할 이름 입력 : ");
	read_line(keyword, NAME_LEN);	// reade_line으로 키워드를 입력받기
	...
	for (int i = 0; i < count; i++) {
		...
		// ex) 길동 > 홍길동, 동 > 홍길동
		if (strstr(contacts[i].name, keyword) != NULL) {	// 일치하는게 있으면
			printf("순번 : %d\n", i + 1);
			print_contact(i);	// 상세 출력
			puts("----------------------------------------");
			found = 1;
		}
	}
...
static void update_contact() {
	...
	read_line(buf, NAME_LEN);	// 엔터만 치면 buf[0]에 \0을 넣게됨
	if (buf[0]!='\0') {	// 이 조건을 걸어 값이 입력되었을때만 메모리를 덮어쓰게함
		// strncpy는 복사할 문자열이 대상 버퍼크기와 같거나 크면 끝에 \0을 붙이지 않는 특징이 있음
		strncpy(contacts[index - 1].name, buf, NAME_LEN);
		contacts[index - 1].name[NAME_LEN - 1] = '\0';	// 안정빵용 추가. 글자를 MAX 적으면 \0이 들어가야 문자열의 끝을 알려줌

	}
	
	printf("전화 (%s) > ", contacts[index - 1].phone);
	read_line(buf, PHONE_LEN);
	if (buf[0] != '\0') {	// 엔터를 친게 아니면, 내용을 입력했으면
...
static int save_contacts(const char* filename) {
	FILE* fp = fopen(filename, "w");	// 쓰기 모드로 오픈
	int i;

	if (!fp) {	// fp == NULL 이면
		puts("파일 저장 실패!(경로나 권한 확인요망)");
		return 0;
	}
...
static int load_contacts(const char* filename) {
	...
	while (fgets(line, sizeof(line), fp) != NULL) {
		...
		if (!ab_ensure_capacity(count + 1)) break; // 용량이 부족하면 자동으로 확장
