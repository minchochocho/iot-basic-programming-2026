...
4. 문자열과 포인터
    - 문자열 연산 함수 : 자주 사용하는 문자열 처리 내장 함수
        ...
        - strcspn() : 문자열에서 찾고자하는 문자열에 포함된 문자를 검색, 일치할때까지 읽어들인 문자의 갯수를 리턴
        - strtok() : 패턴문자열로 기존 문자열을 자르는 함수
        - strchr() : 문자열에서 특정 문자가 가장 먼저 나타나는 곳의 위치를 리턴
...
2. 폴더 및 파일경로
    - 절대경로 - `C:\DEV\IDE\Microsoft VS Code\591199df40\resources\app\licenses`와 같이 드라이브부터 전체 경로를 모두 기재
        - 장점 - 정확한 경로가 표시
        - 단점 - 경로 문자열이 아주 길어질 수 있음

    - 상대경로 `.\LICENSE\rtf`와 같이, .(현재폴더), ..(부모폴더) 기호를 사용해서 짧게 경로를 표시하는 방법
        - 장점 - 경로를 짧게 표시가능
        - 단점 - 개발자 직접 경로가 어떤식으로구성되는지 파악 필요. 같은 폴더에 존재하는 경우 사용 가능
        - 실제로 많이씀

3. 파일 입출력
    - 키보드 등 입력은 버퍼를 통한 입력 스트림(stream에), 출력은 출력 스트림(stream)에 보관되어 있다가
    - 파일, 콘솔 등에 전달
    ![alt text](image-27.png)

    - fgetc() - 파일에서 글자 파일의 마지막(EOF - End Of File)까지 읽는 함수
    ...
    - fprintf() - 포맷에 맞춰서 파일에서 쓰기 함수
    - stdio.h - stdin(표준입력), stdout(표준출력),stderr(표준에러) 포함
    - 인코딩방식 - EUC-KR(2byte로 한글 표현) UTF-8(한글자 최대 6byte, 한글은 3byte)

4. 파일 입출력 모드 - 바이너리, 텍스트 : [소스](./Day09/Chap18-Solution/Chap18-06-app/main.c)
    - r = 읽기모드(read)
    - w = 쓰기모드(write)
    - a - 추가(append) 모드
    - t - 텍스트 모드(생략가능)
    - b - 이진(binary) 모드
    - + - 복합모드
    - 위의 단어를 조합해서 사요ㅕㅇ - "rb+", "r+", etc ...
    - 경로를 표시하는 문자열에서 폴더 구분을 기존 "\\" 와 유닉스, 리눅스, 맥OS 공통의 구분인 "/" 사용 가능


5. 한글 깨짐 현상 : [소스](./Day09/Chap18-Solution/Chap18-03-app/main.c)
...
6. 파일 핸들링 현상
    ...
    - fseek() 함수 사용시 : [소스](./Day09/Chap18-Solution/Chap18-04-app/main.c)
        - SEEK_SET - 파일의 최초 위치
        - SEEK_CUR - 파일의 현재 위치
        - SEEK_END - 파일의 마지막 위치
...
8. 분할 컴파일 : [소스](./Day09/Chap19-Solution/Chap19-04-app/main.c)
    - 여러파일을 분할해서 팀프로젝트나 프로그램의 크기를 줄이기, 코드 재활용 등
...
10. 문자열 처리함수 목록
    - `strcpy()` : 문자열 대입함수
    - strncpy() : 문자열 대입시 글자 갯수만큼 대입하는 함수
    -  str
